---
title: "CFS Recall (Master's Expt 2)"
author: "pss"
...
d <- lapply(files, read_csv, col_types=cols(pas_1=col_character(),pas_2=col_character(), pas = col_character())) %>% 
  bind_rows() %>%
  select(subject, pas, list, trial_test, item_test, tType_test, name_test, gonogo_answer, response_cue, rt_cue, response_noise, rt_noise) %>%
  # select(subject, pas_1, pas_2, list, trial_test, item_test, tType_test, name_test, gonogo_answer, response_cue, rt_cue, response_noise, rt_noise) %>%
  ...
  mutate(pas = as.numeric(substring(pas,1,1))) %>%
  mutate(pas = if_else(is.na(pas),0,pas)) %>%
...
9 participants showed up (out of about 12). 1 left 20 minutes into the experiment. 2 appear not to have tried to name any items. This leaves only 6. All were recruited through SONA. The 120 items per participant, encountered in lists of 12 items. With three conditions (binocular, cfs, not-studied), there were four items in each condition in each list. Of those four items, three were encountered in a “go” test trial, and one was in a “no-go” test trial. So, 1/4 of test-trials were no-go. This leaves up to 30 items in go trials per condition, per participant. 
...
The primary goal of these modification was in improve naming performance for items that were studied binocularly. So far, there isn't much evidence that this has happened. But, I had hoped to get about double this number before making a call for whether learning was improved. 
