...
# 
# grid <- rt_quant %>%
#   modelr::data_grid(Condition, Percentile = seq_range(Percentile, 4), cue_correct_fct, .model = rt_mod) %>%
#   modelr::add_predictions(rt_mod, var = "RT")
# 
# rt_quant %>%
#   ggplot(aes(x=Percentile, y=RT, color=Condition, linetype = cue_correct_fct)) +
#   stat_summary(fun.data = 'mean_cl_boot') +
...
#   scale_color_manual(values=colmap, name= "Condition")+
#   geom_point(data = grid, aes(y = RT), colour = "red", size = 2)
# 
# ggplot(rt_quant, aes(x = Percentile, color = Condition)) +
#   facet_wrap(~cue_correct_fct) +
#   geom_point(aes(y = RT)) +
#   
# 
# rt_quant %>% modelr::add_residuals(rt_mod) %>%
#   ggplot(aes(x = Percentile, y = resid, color = Condition)) +
#   geom_point() +
#   facet_wrap(~cue_correct_fct)
# 
