...
#   unnest() %>%
#   ungroup() %>%
#   mutate(Percentile = ordered(Percentile),
#          Condition = factor(Condition))
  
rt_mod0 <- afex::mixed(RT ~ Condition*Percentile*cue_correct_fct + (1 | subject), data = rt_quant0, type = 3)
# rt_mod <- afex::mixed(RT ~ Condition*Percentile*cue_correct_fct + (1 | subject), data = rt_quant, type = 2)
...
# m <- lsmeans::lsmeans(rt_mod, ~Condition|cue_correct_fct)
# m1 <- lsmeans::lsmeans(rt_mod, ~Condition+Percentile|cue_correct_fct)
```
