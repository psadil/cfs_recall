...
tibble::tibble(rt = seq(from = -1, to = 5, length.out = 1000)) %>%
  ...
  mutate(mean_of_gaussian = factor(mean)) %>%
  ggplot2::ggplot(aes(x = rt, y = density, color = mean_of_gaussian)) +
...
In contrast, the 3-parameter Weibull distribution does have a hard leading edge (there is a 2-parameter Weibull that seems to be more common, but the thrid parameter governs the location of a lower bound, below which the distribution has no density). The other two parameters are shape and scale parameters, whose affects can be seen below.
...
tibble::tibble(rt = seq(from = -1.5, to = 5, length.out = 1000)) %>%
  ...
  ggplot2::ggplot(aes(x = rt, y = density, color = location)) +
...
All that is to say is that it seems like a lack of a hard lower bound isn't necessarily a reason to avoid the exgaussian. I suppose it stil feels a little odd to use a distribution which places density on RTs below 0, but that could changed by at least truncating the exgaussian distribution at 0. Using a lower bound of 0 does not change the results (that studing an item shifts the mean of the gaussian component of the distribution). I haven't tried fitting a model with the Weibull distribution.
