...
%%
% tests for imfindcircles

% [centers, radii, metric] = cellfun(@(x) imfindcircles(x,80,'Sensitivity',.9999999), alpha1, 'UniformOutput', false);
% centers = cellfun(@(x) x(1,:), centers, 'UniformOutput', false);
% radii = cellfun(@(x) x(1), radii, 'UniformOutput', false);
